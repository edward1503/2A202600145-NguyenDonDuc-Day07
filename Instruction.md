...
Bạn có thể visualize luồng RAG và so sánh 2 chunking methods bằng web app HTML:
...
LLM mode trong UI:
- `demo`: dùng mock answer để minh họa flow.
- `real (OpenAI)`: gọi model thật nếu có `OPENAI_API_KEY` trong môi trường hoặc `.env`.
...
Hệ thống hiện có thể đọc PDF theo kiểu optional dependency:
...
### Build và ingest trong UI

- `Build fresh`: clear toàn bộ Chroma collections của app, rồi index lại từ sample data và file bạn upload.
- `Ingest more`: chỉ thêm file mới vào index hiện tại theo checksum, không reset dữ liệu cũ.
- `Single mode`: chỉ có 1 workspace chunking.
- `Compare mode`: mở thêm workspace B để so sánh retrieval/answer side-by-side.

### End-to-end plan

Xem blueprint chi tiết tại [docs/END_TO_END_PIPELINE_PLAN.md](docs/END_TO_END_PIPELINE_PLAN.md).
